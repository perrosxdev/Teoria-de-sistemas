# 🏆 Evaluación

[← Volver al README principal](../README.md)

---

## Composición de la Nota del Proyecto

| Componente | Ponderación |
|-----------|-------------|
| 📄 Informe escrito | 75% |
| 🎤 Presentación en clases | 25% |

---

## Peso en la Nota Final del Curso

> Esta evaluación representa el **40% de la nota final del curso**.

---

> Ver requisitos del informe en [Estructura mínima del informe](./estructura-informe.md)  
> Ver requisitos de la presentación en [Presentación en clases](./presentacion.md)
