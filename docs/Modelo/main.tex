...
\begin{tikzpicture}[
  ...
  lbl/.style = {midway, font=\scriptsize\bfseries},
]
...
% Flechas
\draw[arr] (DB) -- node[lbl, above] {$+$} (DT);
\draw[arr] (DT) -- node[lbl, right] {$+$} (TD);
\draw[arr] (CR) -- node[lbl, right] {$+$} (TD);
\draw[arr] (SH) -- node[lbl, above] {$+$} (TD);
\draw[arr] (TD) -- node[lbl, right] {$-$} (TI);
\draw[arr] (DT) -- node[lbl, left]  {$+$} (TI);
\draw[arr] (TI) -- node[lbl, right] {$+$} (TF);
\draw[arr] (TF) -- node[lbl, above] {$+$} (TP);
\draw[arr] (TP) -- node[lbl, right] {$-$} (DB);
\draw[arr] (DB) -- node[lbl, right] {$+$} (TC);
\draw[arr] (FM) -- node[lbl, above] {$+$} (TC);
\draw[arr] (TC) -- node[lbl, right] {$+$} (DB);
\draw[arr] (MP) -- node[lbl, right] {$-$} (FM);
\draw[arr] (DB) to[bend right=25] node[lbl, left] {$+$} (FM);
\draw[arr] (TD) -- node[lbl, above] {$+$} (IV);
\draw[arr] (IV) -- node[lbl, right] {$+$} (MA);
\draw[arr] (MA) -- node[lbl, above] {$+$} (GT);
\draw[arr] (GT) -- node[lbl, right] {$+$} (VC);
\draw[arr] (VC) to[bend left=40] node[lbl, right] {$+$} (CR);
...
\node[flow]   (f_capt) at (2.3, 2) {Tasa captación};
...
\node[flow]   (f_comp) at (-2.3,-1){Tasa compra};
\node[stock]  (SH)     at (0, -1)  {Stock huevos};
\node[flow]   (f_desp) at (2.3,-1) {Tasa despacho};
...
% ── Subsistema Finanzas ─────────────────────────────────────────────────────
\node[stock]  (MA) at (7, 0.5)  {Margen acum.};
\node[flow]   (f_ing) at (5.5, 0.5) {Ingresos};
\node[cloud]  (c_ing) at (4, 0.5)   {$\infty$};
\draw[flowarr] (c_ing) -- (f_ing) -- (MA);

% ── Subsistema Capacidad ────────────────────────────────────────────────────
\node[stock]  (CR) at (0, -4)   {Cap. Reparto};
\node[flow]   (f_inv) at (-2.3,-4){Inv. capacidad};
\node[cloud]  (c_inv) at (-4,-4)  {$\infty$};
\draw[flowarr] (c_inv) -- (f_inv) -- (CR);

% ── Variables auxiliares ────────────────────────────────────────────────────
\node[aux] (TI) at (2.3, -2.5) {Tasa incumpl.};
\node[aux] (DT) at (0, 0.6)    {Demanda total};
\node[aux] (FM) at (-2, 3.5)   {Fracc. mercado};
...
En este escenario el negocio opera con la capacidad de reparto inicial de 720~cajas/mes durante todo el horizonte de 96~meses. No se realiza ninguna inversión en infraestructura logística. Este escenario sirve como referencia para evaluar el costo real de no actuar.
...
\begin{enumerate}
  \item \textbf{Los retardos amplifican el daño.} El bucle~B2 actúa con retardo porque la pérdida de clientes no es inmediata. Cuatro meses de diferencia en la compra generan 34~meses de diferencia en la recuperación del ROI y 46~M~CLP de diferencia en el margen acumulado final.
  ...
  \item \textbf{El ROI subestima el beneficio sistémico total.} El cálculo de ROI del camión contabiliza solo el beneficio incremental directo atribuible a la capacidad extra. El beneficio de evitar la erosión de la cartera (diferencia de 317,6~cajas/mes de demanda entre E0 y E2 al mes~96) no se refleja en el ROI pero sí en el margen acumulado.

  \item \textbf{La polaridad dominante cambia con el tiempo.} En los primeros meses domina R1 (crecimiento orgánico). Sin intervención, B2 toma progresivamente el control. La inversión proactiva evita ese cambio de polaridad dominante manteniendo el incumplimiento bajo el umbral de activación de la fuga logística.
\end{enumerate}
